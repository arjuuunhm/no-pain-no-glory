...
## Status: current plan stands, nothing here is adopted
...
Reconsider this document when any of the following is true:

- A first model exists and produces walk-forward results against benchmark #1
  (last season regressed to the mean).
- Quantile outputs exist — at that point CRPS is nearly free and the argument
  for adopting it gets much stronger.
- Two candidate models are close enough on RMSE that the ranking between them
  is in doubt. That is the moment the paired-bootstrap point above stops
  being theoretical.
